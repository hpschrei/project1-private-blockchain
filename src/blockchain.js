    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (!isValid) {
                    throw new Error("The new block is not valid: it cannot be added to the chain.")
                }
                ...
                if (errorLog.length > 0) {
                    ...
                    self.height--;
                    throw new Error(['The block chain is compromised: The new block at position <', rollBackedBlock.height, '> will be back-rolled!'].join(""));
                }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (currentTime - messageTime >= 5 * 60) {
                    let error = new Error('This star cannot be added anymore after the allotted 5 minutes. Please try again requesting new ownership verification for it.');
                    console.error(error);
                    reject(error);
                    return;
                }
                ...
                if (!bitcoinMessage.verify(message, address, signature)) {
                    let error = new Error('Your signature is not valid.');
                    console.error(error);
                    reject(error);
                    return;
                }
